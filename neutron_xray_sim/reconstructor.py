...
def reconstruct(
    ...
    for s_idx in range(N_slice):
        ...
        # ── Standard ASTRA iterative algorithms ──────────────────────────
            ...
            sino_id   = astra.data2d.create("-sino", proj_geom, sino2d)
            rec_id    = astra.data2d.create("-vol", vol_geom)
            cfg       = astra.astra_dict("SART_CUDA")    # ASTRA uses SART_CUDA
            cfg["ProjectionDataId"]     = sino_id
            cfg["ReconstructionDataId"] = rec_id
            cfg["ProjectionOrder"]      = "random"       # ordered-subset effect
            alg_id = astra.algorithm.create(cfg)
            # Run n_iter passes over n_subsets subsets each
            astra.algorithm.run(alg_id, n_iter * n_subsets)
            slice_recon = astra.data2d.get(rec_id)
            astra.algorithm.delete(alg_id)
            astra.data2d.delete([sino_id, rec_id])
